...
- **Testing:** `pytest` (backend) and Node.js built-in `node:test` (frontend, including comprehensive test suites for utilities like `api.js`, `models.js`, `websocket.js`, and `chat.js` by simulating DOM manipulation via `global.document` mocking without external libraries) with heavily type-hinted, asynchronous code.
...
- **Error Handling & Validation:** Enforce explicit typed domain exceptions (e.g., catching `OSError` and chaining `from e` to preserve tracebacks) mapping to precise HTTP status codes. Use shared base request models to reduce duplication in API contracts. Use Pydantic models for inbound validation/config with empty env var filtering.
