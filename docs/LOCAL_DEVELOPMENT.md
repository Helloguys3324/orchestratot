...
To configure GitHub Actions (including App Installation, API keys, Repository Secrets, and Action Permissions), see the **[Required GitHub Setup in AI Factory Operations](AI_FACTORY.md#required-github-setup)**. At a minimum, ensure `JULES_API_KEY` and `GH_PAT` are added as **Repository Secrets** (do NOT configure them as 'Environment Secrets' or 'Repository Variables').
