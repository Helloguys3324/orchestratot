...
2. **Configure Credentials**: Your local `.env` file is meant for development. Open `.env` and replace placeholders with your actual keys. Key environment variables include:
   ...
   **Configuration Handling Details:**
   ...
   - **Environment Variables**: The backend utilizes `pydantic-settings` to safely load configuration, prioritizing environment variables over `.env` variables and JSON defaults. Empty strings are typically ignored (via `env_ignore_empty=True`), falling back to defaults. Note that custom field validators may be implemented internally to prevent whitespace-only strings passed from the UI from accidentally overwriting these defaults.
   - **Adding New Configuration**: When adding a new configuration variable, ensure you map it in `backend/config.py` using `pydantic-settings` and provide an explicit template placeholder in `.env.example` so other contributors know it exists.
