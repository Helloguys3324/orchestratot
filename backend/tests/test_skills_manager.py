...
def test_delete_skill_malformed(manager):
    ...
    with pytest.raises(SkillInstallError, match="Failed to delete skill"):
...
def test_install_from_url_failure(mock_client_class, manager):
    ...
    with pytest.raises(SkillInstallError, match="Failed to download skill: 404 Not Found") as exc:
...
def test_install_from_url_request_error(mock_client_class, manager):
    ...
    with pytest.raises(SkillInstallError, match="Failed to download skill: Connection failed") as exc:
        asyncio.run(manager.install_from_url("http://example.com/404.py"))
...
def test_delete_skill_generic_exception(mock_dir, manager):
    ...
    with pytest.raises(SkillInstallError, match="Failed to delete skill: Delete error"):
        manager.delete_skill("custom_1")
...
def test_add_custom_skill_unexpected_error(manager):
    ...
    with pytest.raises(SkillInstallError, match="Failed to create skill"):
        manager._add_custom_skill("custom", {"name": "test"}, "custom", "custom")
